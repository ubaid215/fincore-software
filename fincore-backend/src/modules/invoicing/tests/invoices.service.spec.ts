...
/** Normalize Prisma Decimal string for comparison (Prisma 7 may omit trailing zeros). */
const d4 = (v: unknown) => new Decimal(String(v)).toFixed(4);
...
describe('InvoicesService', () => {
  ...
  describe('create()', () => {
    ...
    it('computes line item total correctly: 40 × 5000 × 1.17 = 234,000', async () => {
      ...
      const { data } = getCreateCall();
      expect(d4(data.totalAmount)).toBe('234000.0000');
      expect(d4(data.subtotal)).toBe('200000.0000');
      expect(d4(data.taxAmount)).toBe('34000.0000');
    });
    ...
    it('calculates discount correctly: 40 × 5000 × (1-0.10) × (1+0.17)', async () => {
      ...
      const { data } = getCreateCall();
      expect(d4(data.totalAmount)).toBe('210600.0000');
      expect(d4(data.discountAmount)).toBe('20000.0000');
      expect(d4(data.taxAmount)).toBe('30600.0000');
    });
    ...
    it('handles floating-point precision: 3 × 0.1 PKR = 0.3000', async () => {
      ...
      const { data } = getCreateCall();
      expect(d4(data.totalAmount)).toBe('0.3000');
    });
    ...
    it('creates multi-line invoice with aggregated totals', async () => {
      ...
      const { data } = getCreateCall();
      expect(d4(data.totalAmount)).toBe('236000.0000');
  ...
  describe('void()', () => {
    ...
    it('throws ConflictException when voiding a PAID invoice (terminal — no VOID transition)', async () => {
      mockPrisma.invoice.findFirst.mockResolvedValue(makeInvoice({ status: InvoiceStatus.PAID }));

      const err = await service.void(ORG_ID, 'inv-001').catch((e: unknown) => e);
      expect(err).toBeInstanceOf(ConflictException);
      expect((err as ConflictException).message).toMatch(/PAID.*VOID|terminal/i);
    });
  ...
  describe('recordPayment()', () => {
    ...
    it('partial payment → status becomes PARTIALLY_PAID', async () => {
      ...
      expect(new Decimal(data.amountPaid as string).toFixed(4)).toBe('117000.0000');
    ...
    it('second partial payment completing the total → PAID', async () => {
      ...
      const { data } = getUpdateCall();
      expect(new Decimal(data.amountPaid as string).toFixed(4)).toBe('234000.0000');
    });
